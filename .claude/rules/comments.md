...
- Doc comments use Google-style docstrings, only for high-level functions or where the name cannot carry the intent; lower-level and test functions are self-explanatory and need none.
- CI yaml, `.toml`, `.j2` and the justfiles have no doc-comment tool, so `#` comments carry the whole rule there, under the same bullets as `.py`.
