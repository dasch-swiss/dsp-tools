...
- Delete: restatements of the code below, history ("previously", "now uses", "was changed to"), and any
  REQ id, user story, or plan-phase reference. A `TODO` without an issue id belongs in the tracker.
...
- This bounds what you *add*, and a deliberate trim applies the same bar to what is already there:
  per sentence, name what a reader breaks without it, cut the ones with no answer, and hold each
  surviving constraint to one or two sentences. Rewrite a constraint, never delete it.
...
- Flipping the tense is not the rewrite: `used to fail the whole run` → `fails the whole run` is simply
  wrong, because the setting being documented is what prevents it. Restate why the code has to be this
  way; where the reason genuinely **is** a past event (a compatibility shim, a workaround kept for old
  servers), state the durable consequence and add a followable pointer — `see #1787`.
