...
- [ ] Added comments follow `CONVENTIONS.md` -> "Comments", in every format and not only `.py`: each states
      a standing constraint rather than what the change discovered, what belongs in the commit body, `docs/`
      or the tracker is not left in the source, an outside-world claim carries a date or a source link, and
      the three carve-outs listed there are not findings. Flag as **rewrite**, never as **delete**
