...
To reuse the project's `default_data_authorship`, 
pass `xmllib.ResourceAuthorshipDefault.PROJECT_DEFAULT` instead of a list.
...
- `apply_default_resource_authorship` on the `XMLRoot` actually writes the authorship onto your resources,
  either from a literal list, 
- or (with `xmllib.ResourceAuthorshipDefault.PROJECT_DEFAULT`) from the project's default resolved at
  `xmlupload`.
