...
Components/tools:
- mkdocstrings: triggered by dedicated instructions in md files, this tool reads docstrings and generates API docs
- mkdocs-gen-files: for every python file, auto-generates a md file with instructions for mkdocstrings
- mkdocs-literate-nav: for every md file auto-generated by mkdocs-gen-files, auto-generates an entry in the nav bar
- mkdocs-section-index: instead of rendering content of __init__.py as "Index" subpage, 
  render it as content of its section header. The (normally unclickable) toggle section headers become clickable.

This entire setup + a template for this script are documented here:
