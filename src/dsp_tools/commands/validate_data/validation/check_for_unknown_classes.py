...
def check_for_unknown_resource_classes(rdf_graphs: RDFGraphs, used_resource_iris: set[str]) -> str | None:
    """
    ...
    """
    if unknown_classes := _check_unknown_classes(rdf_graphs, used_resource_iris):
        return _get_msg_str_unknown_classes_in_data(unknown_classes)
    return None


def _check_unknown_classes(rdf_graphs: RDFGraphs, used_resource_iris: set[str]) -> UnknownClassesInData | None:
    classes_in_onto = _get_all_onto_classes(rdf_graphs)
    if extra_cls := used_resource_iris - classes_in_onto:
        unknown_classes = {reformat_onto_iri(x) for x in extra_cls}
        defined_classes = {reformat_onto_iri(x) for x in classes_in_onto}
