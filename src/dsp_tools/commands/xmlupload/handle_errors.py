...
# XmlUploadInterruptedError exits with 1, a KeyboardInterrupt with 130. BadCredentialsError also exits
# with 1 but is raised natively by the client and re-raised in execute_upload.py, not converted here.
# entry_point.py prints the DSP-TOOLS exceptions but not a KeyboardInterrupt, which is why the keyboard
# interrupt handlers emit their message as a user warning.
