...
@pytest.fixture(scope="module")
def test_directories() -> Generator[dict[str, Path], None, None]:
    """Fixture providing temporary test directories with automatic cleanup."""
    cwd = Path("cwd")
    testdata_tmp = Path("testdata/tmp")
    cwd.mkdir(exist_ok=True)
    testdata_tmp.mkdir(exist_ok=True)
    yield {"cwd": cwd, "testdata_tmp": testdata_tmp}
    shutil.rmtree(cwd)
    shutil.rmtree(testdata_tmp)
...
def test_create_project(creds: ServerCredentials, test_project_systematic_file: Path) -> None:
    success = create(
        project_json=test_project_systematic_file.absolute(),
        creds=creds,
    )
    assert success
...
def test_get_project(
    ...
    _compare_project(project_original, project_returned)
    _compare_groups(
...
def _get_original_project(test_project_systematic_file: Path) -> dict[str, Any]:
    """
    ...
    """
    with open(test_project_systematic_file, encoding="utf-8") as f:
...
def _compare_project(
    ...
    """
    assert list(project_original.keys()) == list(project_returned.keys())
    for field in [
        "shortcode",
        "shortname",
        "longname",
    ...
    assert orig_licenses == ret_licenses, (
        f"Field enabled_licenses is not identical: original={orig_licenses}, returned={ret_licenses}"
    )

    orig_keywords = sorted(project_original["project"]["keywords"])
    ret_keywords = sorted(project_returned["project"]["keywords"])
    assert orig_keywords == ret_keywords, (
        f"Field keywords is not identical: original={orig_keywords}, returned={ret_keywords}"
    )
...
def _compare_groups(
    ...
    if len(groups_original) != len(groups_returned):
        assert groups_original == groups_returned, (
            "Returned number of groups is different from original number of groups."
        )
    else:
        for orig, ret in zip(groups_original, groups_returned):
            assert orig == ret, f"Group with original name '{orig['name']}' and returned name '{ret['name']}' failed."
...
def _compare_users(
    ...
    if len(users_original) != len(users_returned):
        assert users_original == users_returned, "Returned number of users is different from original number of users."
    else:
        for orig, ret in zip(users_original, users_returned):
            assert orig == ret, (
                f"User with original name '{orig['username']}' and returned name '{ret['username']}' failed."
            )
...
def _compare_lists(
    ...
    if len(lists_original) != len(lists_returned):
        assert lists_original == lists_returned, "Returned number of lists is different from original number of lists."
    else:
        for orig, ret in zip(lists_original, lists_returned):
            assert orig == ret, f"List with original name '{orig['name']}' and returned name '{ret['name']}' failed."
...
def _compare_properties(
    ...
    if len(properties_original) != len(properties_returned):
        assert properties_original == properties_returned, (
            f"Onto {onto_name}: Returned number of properties is different from original number of properties."
        )
    else:
        for orig, ret in zip(properties_original, properties_returned):
            assert orig == ret, (
                f"Onto '{onto_name}': Property with original name '{orig['name']}' "
...
def _compare_resources(
    ...
    resources_original = _remove_onto_names_in_original_resources(onto_name, resources_original)

    resources_original, resources_returned = _remove_cardinalities_in_original_resources(
        onto_name, resources_original, resources_returned
    )

    resources_original, resources_returned = _sort_resources(resources_original, resources_returned)

    if len(resources_original) != len(resources_returned):
        assert resources_original == resources_returned, (
            f"Onto {onto_name}: Returned number of resources is different from original number of resources."
        )
    else:
        for orig, ret in zip(resources_original, resources_returned):
            if orig.get("cardinalities") != ret.get("cardinalities"):
                assert orig.get("cardinalities") == ret.get("cardinalities"), (
                    f"Onto '{onto_name}': The cardinalities of resource with original name '{orig['name']}' "
                    f"and returned name '{ret['name']}' failed."
                )
            else:
                assert orig == ret, (
                    f"Onto '{onto_name}': Resource with original name '{orig['name']}' and returned name "
