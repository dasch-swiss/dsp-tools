...
def create_generic_project_9999(creds: ServerCredentials) -> None:
    assert create(Path("testdata/validate-data/core_validation/core-validation-project-9999.json"), creds, False)
...
def _xmlupload_minimal_correct_9999(create_generic_project_9999, creds) -> None:
    ...
    """
    absolute_xml_path = Path("testdata/validate-data/core_validation/minimal_correct.xml").absolute()
...
def _xmlupload_text_parsing_9999(create_generic_project_9999: None, creds: ServerCredentials) -> None:
    ...
    which fails, because the id2iri mapping is named after the shortcode and the timestamp.
    """
    absolute_xml_path = Path("testdata/xml-data/parsing-special-characters-in-richtext-9999.xml").absolute()
    original_cwd = Path.cwd()
    with TemporaryDirectory() as tmpdir:
        with pytest.MonkeyPatch.context() as m:
            m.chdir(tmpdir)
...
def create_4125_e2e_project(creds: ServerCredentials) -> None:
    assert create(Path("testdata/json-project/generic-e2e-project-4125.json"), creds, DO_NOT_EXIT_IF_EXISTS)
