...
def data_systematic_cleaned() -> etree._Element:
    return prepare_xml_file(Path("testdata/xml-data/test-data-systematic.xml"))
...
def test_parse_and_clean_xml_file_same_regardless_of_input(data_systematic_unclean: Path) -> None:
    from_tree = prepare_xml_file(data_systematic_unclean)
    from_file = prepare_xml_file(Path("testdata/xml-data/test-data-systematic.xml"))
