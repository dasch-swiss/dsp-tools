...
def data_systematic_cleaned() -> etree._Element:
    return parse_clean_and_validate_xml(Path("testdata/xml-data/test-data-systematic.xml"))
...
def test_parse_and_clean_xml_file_same_regardless_of_input(data_systematic_unclean: Path) -> None:
    from_tree = parse_clean_and_validate_xml(data_systematic_unclean)
    from_file = parse_clean_and_validate_xml(Path("testdata/xml-data/test-data-systematic.xml"))
