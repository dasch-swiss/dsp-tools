...
def test_rename_deprecated_lang_cols() -> None:
    original_df = pd.DataFrame({"en": [1, 2, 3], "de": [1, 2, 3], "fr": [1, 2, 3], "it": [1, 2, 3], "rm": [1, 2, 3]})
...
def test_do_property_excel_compliance_all_good() -> None:
...
def test_do_property_excel_compliance_problems() -> None:
    original_df = pd.DataFrame(
        {
...
def test_rename_deprecated_hlist() -> None:
    ...
    original_df = pd.DataFrame({"hlist": [pd.NA, pd.NA, "languages"], "gui_attributes": [pd.NA, "attribute_1", pd.NA]})
...
def test_get_final_series_two_series() -> None:
    ...
    assert list(result) == list(expected)  # type: ignore[arg-type]


def test_get_final_series_one_series_one_None() -> None:
    mandatory_check = pd.Series([True, True, False])
...
def test_get_final_series_one_None_one_series() -> None:
...
def test_get_final_series_two_None() -> None:
    mandatory_check = None
    no_attribute_check = None
    expected = None
    result = e2j._get_final_series(mandatory_check, no_attribute_check)
...
def test_row2prop_duplicate_attrib_key_problem() -> None:
