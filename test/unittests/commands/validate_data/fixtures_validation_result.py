...
@pytest.fixture
def report_diverse_link_problems(onto_graph: Graph) -> tuple[Graph, Graph]:
    validation_str = f"""{PREFIXES}
    [ a sh:ValidationResult ;
        sh:detail _:bn_id_link_value_type_violation ;
        sh:focusNode <http://data/id_link_value_type_violation> ;
        sh:resultMessage "Value does not have shape <http://api.knora.org/ontology/knora-api/shapes/v2#LinkValue_ClassShape>" ;
        sh:resultPath onto:testHasLinkTo ;
        sh:resultSeverity sh:Violation ;
        sh:sourceConstraintComponent sh:NodeConstraintComponent ;
        sh:sourceShape onto:testHasLinkTo_PropShape ;
        sh:value <http://data/value_id_link_value_type_violation> ] .

    _:bn_id_link_value_type_violation a sh:ValidationResult ;
        sh:focusNode <http://data/value_id_link_value_type_violation> ;
        sh:resultMessage "LinkValue" ;
        sh:resultSeverity sh:Violation ;
        sh:sourceConstraintComponent sh:ClassConstraintComponent ;
        sh:sourceShape <http://api.knora.org/ontology/knora-api/shapes/v2#LinkValue_ClassShape> ;
        sh:value <http://data/value_id_link_value_type_violation> .
    ...
    validation_g.parse(data=validation_str, format="ttl")
    data_str = f"""{PREFIXES}
    <http://data/id_link_value_type_violation> a onto:ClassWithEverything ;
        rdfs:label "Link Prop"^^xsd:string ;
        onto:testHasLinkTo <http://data/value_id_link_value_type_violation> .
        
    <http://data/value_id_link_value_type_violation> a knora-api:TextValue ;
        knora-api:valueAsString "id_1"^^xsd:string .
