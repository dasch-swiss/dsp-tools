...
def test_construct_audio(file_value_shacl: Graph) -> None:
    result_list = list(file_value_shacl.subjects(SH.property, API_SHAPES.hasAudioFileValue_Cardinality))
...
def test_construct_moving_image(file_value_shacl: Graph) -> None:
    result_list = list(file_value_shacl.subjects(SH.property, API_SHAPES.hasMovingImageFileValue_Cardinality))
    assert len(result_list) == 1
    moving_image = result_list[0]
