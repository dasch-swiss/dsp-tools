...
class TestOneResource:
    ...
    def test_resource_with_authorship(self, lookups: XmlReferenceLookups):
        res = ParsedResource(
            res_id="id",
            res_type=RES_TYPE,
            label="lbl",
    ...
    def test_resource_without_authorship(self, lookups: XmlReferenceLookups):
        res = ParsedResource(
            res_id="id",
            res_type=RES_TYPE,
            label="lbl",
...
class TestValues:
    ...
    def test_list_value_with_iri(self, lookups: XmlReferenceLookups):
        val = ParsedValue(
            HAS_PROP, ("", "http://rdfh.ch/9999/node"), KnoraValueType.LIST_VALUE, "public", "cmt", None, 0
        )
        result = _get_one_processed_value(val, lookups)
        assert isinstance(result, ProcessedList)
        assert result.value == "http://rdfh.ch/9999/node"
    ...
    def test_richtext_value(self, lookups: XmlReferenceLookups):
        ...
        val = ParsedValue(HAS_PROP, text_str, KnoraValueType.RICHTEXT_VALUE, "public", "cmt", None, 0)
        result = _get_one_processed_value(val, lookups)
...
class TestValueOrder:
    ...
    def test_list_value(self, lookups: XmlReferenceLookups):
        val = ParsedValue(HAS_PROP, ("list", "node"), KnoraValueType.LIST_VALUE, "public", "cmt", 1, 0)
        result = _get_one_processed_value(val, lookups)
        assert isinstance(result, ProcessedList)
        assert result.value == "http://rdfh.ch/9999/node"
        assert result.prop_iri == HAS_PROP
    ...
    def test_link_value(self, lookups: XmlReferenceLookups):
        val = ParsedValue(HAS_PROP, "other_id", KnoraValueType.LINK_VALUE, "public", "cmt", 3, 0)
        result = _get_one_processed_value(val, lookups)
        assert isinstance(result, ProcessedLink)
        assert result.value == "other_id"
        assert result.prop_iri == f"{HAS_PROP}Value"
    ...
    def test_region_preview_value(self, lookups: XmlReferenceLookups):
        val = ParsedValue(HAS_PROP, "region_id", KnoraValueType.REGION_PREVIEW_VALUE, "public", "cmt", 3, 0)
        result = _get_one_processed_value(val, lookups)
        assert isinstance(result, ProcessedRegionPreview)
        assert result.value == "region_id"
        assert result.prop_iri == HAS_PROP
    ...
    def test_richtext_value(self, lookups: XmlReferenceLookups):
        text_str = "<text>this is text</text>"
        val = ParsedValue(HAS_PROP, text_str, KnoraValueType.RICHTEXT_VALUE, "public", "cmt", 4, 0)
        result = _get_one_processed_value(val, lookups)
        assert isinstance(result, ProcessedRichtext)
        assert result.value.xmlstr == text_str
        assert result.prop_iri == HAS_PROP
    ...
    def test_uri_value(self, lookups: XmlReferenceLookups):
        ...
        val = ParsedValue(HAS_PROP, "https://dasch.swiss", KnoraValueType.URI_VALUE, None, None, 3, 0)
        result = _get_one_processed_value(val, lookups)
        assert isinstance(result, ProcessedUri)
        assert result.value == "https://dasch.swiss"
        assert result.prop_iri == HAS_PROP
