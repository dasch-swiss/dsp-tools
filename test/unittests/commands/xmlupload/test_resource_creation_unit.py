...
def test_idempotency_on_success() -> None:
    ...
    for _ in range(3):
        _tidy_up_resource_creation_idempotent(upload_state, "foo_1_iri", "foo_1_label", xml_resources[0], 1, 2)
...
def test_idempotency_on_failure() -> None:
    ...
    upload_state = UploadState(xml_resources.copy(), [], IriResolver(), None, UploadConfig(), {})
    for _ in range(3):
        _tidy_up_resource_creation_idempotent(upload_state, None, None, xml_resources[0], 1, 2)
        assert upload_state.pending_resources == xml_resources[1:]
