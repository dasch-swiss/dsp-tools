...
from dsp_tools.utils.validate_xml_against_schema import _find_xml_tags_in_simple_text_elements, validate_xml
...
class TestValidateXMLAgainstSchema(unittest.TestCase):
    def test_data_systematic(self) -> None:
        self.assertTrue(validate_xml(input_file="testdata/xml-data/test-data-systematic.xml"))

    def test_data_minimal(self) -> None:
        self.assertTrue(validate_xml(input_file=etree.parse(source="testdata/xml-data/test-data-minimal.xml")))
...
class TestFindXMLTagsInUTF8(unittest.TestCase):
    def test_find_xml_tags_in_simple_text_elements_all_good(self) -> None:
        ...
        for xml in utf8_texts_with_allowed_html_escapes:
            self.assertTrue(_find_xml_tags_in_simple_text_elements(etree.fromstring(xml)))
